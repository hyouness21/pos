...
{{-- Month picker --}}
<form id="emonthly-filter" method="GET" class="flex gap-2 mb-4">
    <select name="month" onchange="liveFilter('emonthly-filter','emonthly-results',0)"
    ...
    <select name="year" onchange="liveFilter('emonthly-filter','emonthly-results',0)"
...
<div class="grid grid-cols-2 gap-3 mb-5">
    ...
    <div class="bg-red-50 border border-red-100 rounded-2xl p-4">
        ...
        @php $monthStr = sprintf('%04d-%02d', $data['year'], $data['month']); @endphp
        <a href="{{ route('expenses.index', ['month' => $monthStr]) }}"
...
@php
    ...
    $byType = $data['expenses']->groupBy('type');
@endphp
    ...
    @foreach ($byType as $type => $group)
        <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-3 flex items-center gap-3">
            <div class="flex-1 min-w-0">
                <span class="text-xs px-2 py-0.5 rounded-full font-medium {{ $typeColors[$type] ?? 'bg-gray-100 text-gray-600' }}">
                    {{ __($group->first()->typeLabel()) }}
                </span>
                <p class="text-xs text-gray-400 mt-1">{{ $group->count() }} {{ __('records') }}</p>
            </div>
            <p class="font-bold text-red-500 shrink-0">${{ number_format($group->sum('amount'), 2) }}</p>
        </div>
    @endforeach
</div>
@endif
...
@if ($data['rows']->isEmpty())
    <div class="text-center py-16 text-gray-400">
        ...
        <p class="text-sm mt-1">{{ __('Either no sales this month, or items have no cost price set.') }}</p>
    </div>
@else
    {{-- Per-item breakdown --}}
