...
@section('content')
<form method="POST" action="{{ route('items.update', $item) }}" enctype="multipart/form-data" class="space-y-4">
    @csrf @method('PUT')

    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-4 space-y-4">

        @if ($item->image)
            <img src="{{ Storage::url($item->image) }}" class="w-full h-36 object-cover rounded-xl">
        @endif
        ...
        @include('items._barcode_field', ['value' => old('barcode', $item->barcode)])

        {{-- Pricing --}}
        <div x-data="{ cost: '{{ old('cost_price', $item->cost_price) }}', sell: '{{ old('price', $item->price) }}' }"
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input type="number" name="cost_price" x-model="cost"
                               value="{{ old('cost_price', $item->cost_price) }}" step="0.01" min="0" placeholder="0.00"
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input type="number" name="price" x-model="sell"
                               value="{{ old('price', $item->price) }}" step="0.01" min="0" required placeholder="0.00"
                               class="w-full border border-gray-300 rounded-xl pl-7 pr-3 py-2.5 text-sm focus:ring-2 focus:ring-indigo-500 outline-none">
                    </div>
        ...
        <div class="grid grid-cols-2 gap-3">
            ...
            <div>
                ...
                <input type="number" name="low_stock_threshold" value="{{ old('low_stock_threshold', $item->low_stock_threshold) }}" min="0" required
                       class="w-full border border-gray-300 rounded-xl px-4 py-2.5 text-sm focus:ring-2 focus:ring-indigo-500 outline-none">
            </div>
        </div>
